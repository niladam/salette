...
class MockClient
{
    ...
    {
        return empty($this->sequenceResponses)
            && empty($this->connectorResponses)
            && empty($this->requestResponses)
            && empty($this->urlResponses);
    }
    ...
    public function getLastRequest(): ?Request
    {
        ...
        return $lastResponse
            ? $lastResponse->getPendingRequest()->getRequest()
            : null;
    }

    /**
     * Get the last pending request that the mock manager sent.
     */
    public function getLastPendingRequest(): ?PendingRequest
    {
        ...
        return $lastResponse
            ? $lastResponse->getPendingRequest()
            : null;
    }
     ...
     * @throws \ReflectionException
     */
    protected function checkRequestWasSent($request, ?int $index = null): bool
    {
        $passed = false;
