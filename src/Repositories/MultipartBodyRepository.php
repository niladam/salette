...
class MultipartBodyRepository implements BodyRepository, MergeableBody
{
    ...
    {
        $parsedArrays = array_map([$this, 'parseMultipartArray'], $arrays);

        call_user_func_array([$this->data, 'merge'], $parsedArrays);
     ...
     * @return $this
     */
    public function attach(MultipartValue $file): self
    {
    ...
    {
        $multipartValues = array_filter($value, static fn ($item): bool => $item instanceof MultipartValue);

        if (count($value) !== count($multipartValues)) {
            throw new InvalidArgumentException(
                sprintf('The value array must only contain %s objects.', MultipartValue::class)
            );
        }
