...
test('you can use the when method to invoke a callback when a given condition is truthy', function () {
    ...
    expect($request->headers()->all())
        ->toHaveKey('X-Name', 'Sam')
        ->and($request->headers()->all())->not->toHaveKey('X-Name', 'Alex');
});
...
test('you can use the unless method to invoke a callback when a given condition is falsy', function () {
    ...
    expect($request->headers()->all())
        ->toHaveKey('X-Name', 'Alex')
        ->and($request->headers()->all())->not->toHaveKey('X-Name', 'Sam');
});
...
test('it will pass the condition value as the second argument of the callable', function () {
    ...
    $request->when(true, function (UserRequest $request, $value) {
        expect($value)
            ->toBeBool()
            ->and($value)->toBeTrue();
    });

    $request->unless(false, function (UserRequest $request, $value) {
        expect($value)
            ->toBeBool()
            ->and($value)->toBeFalse();
    });
