...
test('you can check if a file exists', function () {
    ...
    expect($storage->exists('Pest.php'))
        ->toBeTrue()
        ->and($storage->missing('Pest.php'))->toBeFalse();
});

test('you can check if a file is missing', function () {
    $storage = new Storage('tests');

    expect($storage->exists('HelloWorld.php'))
        ->toBeFalse()
        ->and($storage->missing('HelloWorld.php'))->toBeTrue();
});
...
test('it will create a file with nested folders', function () {
    ...
    $path =
        'Testing' . DIRECTORY_SEPARATOR . 'some' . DIRECTORY_SEPARATOR
        . 'other' . DIRECTORY_SEPARATOR . 'directories' . DIRECTORY_SEPARATOR . 'example.txt';
    ...
    expect($storage->exists($path))
        ->toBeTrue()
        ->and($storage->get($path))->toEqual('Hello World');
});
