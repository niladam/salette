...
use Salette\Tests\Fixtures\Exceptions\TestResponseException;
...
test('you can create sequence mocks', function () {
    ...
    expect($mockClient->getNextFromSequence())
        ->toEqual($responseA)
        ->and($mockClient->getNextFromSequence())->toEqual($responseB)
...
test('you can create connector mocks', function () {
    ...
    expect($mockClient->guessNextResponse($connectorA->createPendingRequest($connectorARequest)))
        ->toEqual($responseA)
        ->and($mockClient->guessNextResponse($connectorB->createPendingRequest($connectorBRequest)))
        ->toEqual($responseB)
        ->and($mockClient->isEmpty())->toBeFalse();
});
...
test('you can create url mocks', function () {
    ...
    expect($mockClient->guessNextResponse($connectorA->createPendingRequest($requestA)))
        ->toEqual($responseA)
        ->and($mockClient->guessNextResponse($connectorA->createPendingRequest($requestB)))->toEqual($responseB)
        ->and($mockClient->guessNextResponse($connectorB->createPendingRequest($requestC)))->toEqual($responseC);
});
...
test('saloon throws an exception if it cant work out the url response', function () {
    ...
    $this->expectExceptionMessage(
        'Salette was unable to guess a mock response for your request [https://google.com/user], 
...
test('you can mock exceptions with a closure', function () {
    $mockClient = new MockClient([
        MockResponse::make(['name' => 'Sam']),
        MockResponse::make(['name' => 'Patrick'])
            ->throw(fn ($pendingRequest) => new TestResponseException('Unable to connect!', $pendingRequest)),
    ]);
