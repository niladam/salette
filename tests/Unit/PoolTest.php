...
it('accepts a generator for requests', function () {
    ...
    expect($generatorCallback)
        ->toBeCallable()
        ->and($generatorCallback())->toBeInstanceOf(Generator::class);
...
it('accepts a callback that returns an array for requests', function () {
    ...
    expect($arrayCallback)
        ->toBeCallable()
        ->and($requests->all())->toBeArray();
...
it('accepts a callback that returns an array for aliased requests', function () {
    ...
    expect($count)
        ->toBe(3)
        ->and($requests->all())->toHaveKeys(['a', 'b', 'c']);
});
