...
use Salette\Tests\Fixtures\Requests\SubRequest;
use Salette\Tests\Fixtures\Requests\UserRequestWithBootPlugin;
...
test('a plugin boot method has access to the request', function () {
    ...
    expect($headers)
        ->toHaveKey('X-Plugin-User-Id', 1)
        ->and($headers)->toHaveKey('X-Plugin-Group-Id', 2);
});
